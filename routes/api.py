...
def assistant_chat():
    """Handle AI Assistant chat requests with Enhanced Memory and Actions"""
    try:
        data = request.json
        message = data.get('message')
        context = data.get('context', {})
        history = data.get('history', [])  # Support for conversation history
        
        if not message:
            return jsonify({"success": False, "error": "No message provided"}), 400
            
        logging.info(f"AI Assistant Chat: {message[:50]}... (History: {len(history)} msgs)")
        
        # Construct system prompt based on context
        system_prompt = """
        You are the PhoVite AI Assistant, a highly advanced, empathetic, and intelligent creative partner.
        Your goal is to assist users in creating stunning invitations, navigating the platform, and answering questions.
        
        You have control over the website's interface and can perform actions.
        ...
        - Event types: Weddings, Birthdays, Parties, Corporate Events, Baby Showers, etc.
        - Vibes/Styles: Neon, Royal, Minimal, Floral, Retro, Cyberpunk, Watercolor, etc.
        
        Tone: Friendly, enthusiastic, professional, and concise. Use emojis effectively.
        """
        
        if context.get('context') == 'creating_invitation':
            system_prompt += "\nThe user is currently on the creation page. You can actively help them fill out the form fields."
        elif context.get('context') == 'viewing_dashboard':
            system_prompt += "\nThe user is on their dashboard. Help them manage invites or start new ones."
        elif context.get('context') == 'landing_page':
            system_prompt += "\nThe user is on the home page. Guide them to 'Create Invitation'."
            
        system_prompt += f"\nCurrent Page URL: {context.get('url', 'unknown')}"
        ...
        JSON Structure:
        {
            ...
            "action": {
                "type": "navigate" | "fill_form" | "trigger_generate" | "change_theme" | "copy_text" | "none",
                "payload": {} 
            }
        }
        ...
        2. type: "fill_form"
           payload: {
               "eventType": "...", "celebrantName": "...", "eventDate": "...", 
               "eventTime": "...", "eventVenue": "...", "eventMessage": "...", 
               "vibe": "..." 
           }
           Use when user provides event details. intelligently extract info.
           
        3. type: "trigger_generate"
           payload: {}
           Use ONLY when user explicitly confirms they are ready to generate/create the final invitation.
           
        4. type: "change_theme"
           payload: { "theme": "dark" | "light" }
           Use when user asks to switch to dark mode, light mode, or "lights off".
           
        5. type: "copy_text"
           payload: { "text": "The text to copy" }
           Use when generating hashtags, poems, or messages that the user might want to copy.
        ...
        return jsonify({
            "success": True,
            "response": ai_response.get('response', 'I am processing your request...'),
